    ...
    $("#twisterContainer li").each(function () {
      ...
      fetch(url, { signal: newItemDetailsAbort.signal })
        ...
        .then((html) => {
          ...
          // Always parse the price because we're here and it could be more
          // accurate. It's only an error if we fail to parse the price AND
          // there wasn't already a price.
          var foundPrice = _findPagePriceInHtml(htmlDoc);
          var $primeHtml = $(
          ...
          // Display results
          if (null !== count) {
            addEstimatedToElement(foundPrice.pennies, count, $addElementsTo, 1);
          }
          $loading.text("");
